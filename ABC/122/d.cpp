...
int main() {
    ...
    mint dp[n+1][4][4][4];
    dp[0][3][3][3] = 1;
    REP(i,n) REP(w,4) REP(x,4) REP(y,4) REP(z,4) {
        if(z==2 && y==1 && x==0) continue;
        if(z==1 && y==2 && x==0) continue;
        if(z==2 && y==0 && x==1) continue;
        if(z==2 && x==1 && w==0) continue;
        if(z==2 && y==1 && w==0) continue;
